...
public class GetListSpotTrafficView extends AsyncTask<Double, Void, String>{
	...
	public GetListSpotTrafficView(SpotAdapter ad, ArrayList<Spot> arrayl) {
	...
	protected void onPreExecute() {
		...
		super.onPreExecute();
		TrafficView.pb.setVisibility(View.VISIBLE);
	}
	...
	protected String doInBackground(Double... location) {
		...
		Log.d("start index:", TrafficView.start+"");
		return MyTool.readTextFromUrl_(Server.LIST_ALL_SPOT+TrafficView.start+"/"+TrafficView.num_row+"/"+location[0]+"/"+location[1]);  //show only 5 spots near 
	}
	...
	protected void onPostExecute(String result) {
		...
		try {
			...
			if (morespot.getString("morespot").equals("0")) {
				TrafficView.btnLoadMore.setVisibility(View.GONE);
			}
			...
			loadDone = true;
			TrafficView.pb.setVisibility(View.INVISIBLE);
			
			if (TrafficView.start==1) {
				TrafficView.start = TrafficView.num_row+1;
			}
			else
			{ 
				TrafficView.start = TrafficView.start+TrafficView.num_row;
			}
			
			Log.d("Value start after Async Get More:", TrafficView.start+"");
